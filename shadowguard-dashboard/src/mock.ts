...
/** Alert createdAt used to be hardcoded absolute dates (2026-08-07..11). That
 * silently broke Trends & Policy: computeTrends (trends.ts) filters to a
 * trailing 7-day window measured against the real Date.now(), so as real
 * calendar time passed the fixed dates, they aged out one by one until none
 * were left and the page went empty -- not a threshold-logic bug, a data-
 * staleness one (see the mini-donut-overflow-adjacent diagnosis this was
 * found alongside).
 *
 * Fix: compute createdAt relative to Date.now() at module load instead, so
 * it's re-anchored fresh on every real page load and never calendar-drifts
 * out of the window again. hoursAgo() offsets below preserve the EXACT
 * relative spacing the original hardcoded dates had to each other (oldest
 * to newest spans ~88h10m/3d16h) -- same relative shape, just re-anchored
 * with the newest alert 3h ago instead of pinned to a specific past date, so
 * the whole cluster always sits comfortably inside any trailing-7-day
 * window with room to spare, rather than living right at its edge.
 *
 * This does NOT make alerts immortal -- computeTrends's own recency filter
 * is untouched and still correctly excludes alerts once they're actually
 * older than 7 days relative to whatever `now` it's evaluated against (see
 * the aging-simulation check that passes a future `now` directly into
 * computeTrends rather than waiting on the calendar). */
const hoursAgo = (h: number) => new Date(Date.now() - h * HOUR_MS).toISOString();

export const alerts: Alert[] = [
 { id:'AL-1042', userId:'u-1', userName:'Maya Chen', department:'Marketing', severity:'high', score:78, app:'Perplexity AI', category:'Shadow AI', status:'new', createdAt:hoursAgo(31.4333), tier:'R3', recommendation:'Flag for IT review. Confirm whether a sanctioned research tool meets the team’s need.', evidence:[{label:'New application observed',detail:'Perplexity AI is not in the approved application inventory.',observedAt:'09:14',strength:'observed'},{label:'Usage outside baseline',detail:'Activity occurred 3 hours after Maya’s established work window.',observedAt:'09:14',strength:'observed'},{label:'Peer context',detail:'12% of Marketing has used this app; below evaluation threshold.',observedAt:'09:15',strength:'context'}] },
 { id:'AL-1041', userId:'u-2', userName:'Jordan Rivers', department:'Sales', severity:'critical', score:93, app:'Personal Google Drive', category:'Personal cloud', status:'under_review', createdAt:hoursAgo(38.3667), tier:'R6', recommendation:'Escalate to security for human review. Do not take automated enforcement action.', evidence:[{label:'Volume anomaly',detail:'Upload volume is 4.8× the 30-day user baseline.',observedAt:'02:18',strength:'observed'},{label:'Time anomaly',detail:'Activity occurred outside the user’s normal work hours.',observedAt:'02:18',strength:'observed'},{label:'Known personal-cloud category',detail:'Destination is categorized as personal cloud storage.',observedAt:'02:19',strength:'context'}] },
 { id:'AL-1039', userId:'u-3', userName:'Alex Morgan', department:'Sales', severity:'medium', score:66, app:'Notion AI', category:'Shadow AI', status:'new', createdAt:hoursAgo(49.1167), tier:'R4', recommendation:'Recommend sanctioned-alternative evaluation because use is widespread in this peer group.', evidence:[{label:'Peer prevalence',detail:'43% of Sales has active use of this application.',observedAt:'15:34',strength:'observed'},{label:'Unapproved app',detail:'Notion AI is not yet approved for this tenant.',observedAt:'15:33',strength:'observed'}] },
 { id:'AL-1037', userId:'u-1', userName:'Maya Chen', department:'Marketing', severity:'low', score:42, app:'Canva', category:'Unauthorized SaaS', status:'resolved', createdAt:hoursAgo(77.3333), tier:'R2', recommendation:'Notify employee and offer a self-audit prompt.', evidence:[{label:'First observation',detail:'First observed use in the current review window.',observedAt:'11:20',strength:'observed'}] },
 ...
 { id:'AL-1031', userId:'u-4', userName:'Priya Nair', department:'Engineering', severity:'low', score:24, app:'Notion AI', category:'Shadow AI', status:'new', createdAt:hoursAgo(3), tier:'R2', recommendation:'Notify employee and confirm whether a sanctioned research tool meets the team’s need.', evidence:[{label:'New application observed',detail:'Notion AI is not in the approved application inventory.',observedAt:'13:40',strength:'observed'}] },
 { id:'AL-1030', userId:'u-5', userName:'Sam Osei', department:'Engineering', severity:'low', score:29, app:'Perplexity AI', category:'Shadow AI', status:'new', createdAt:hoursAgo(22.5833), tier:'R2', recommendation:'Notify employee and confirm whether a sanctioned research tool meets the team’s need.', evidence:[{label:'New application observed',detail:'Perplexity AI is not in the approved application inventory.',observedAt:'18:05',strength:'observed'}] },
 { id:'AL-1029', userId:'u-6', userName:'Dana Kim', department:'Product', severity:'low', score:21, app:'Notion AI', category:'Shadow AI', status:'resolved', createdAt:hoursAgo(55.4667), tier:'R2', recommendation:'Notify employee and confirm whether a sanctioned research tool meets the team’s need.', evidence:[{label:'New application observed',detail:'Notion AI is not in the approved application inventory.',observedAt:'09:12',strength:'observed'}] },
 { id:'AL-1028', userId:'u-7', userName:'Leo Fischer', department:'Product', severity:'medium', score:57, app:'Perplexity AI', category:'Shadow AI', status:'under_review', createdAt:hoursAgo(91.1667), tier:'R3', recommendation:'Recommend sanctioned-alternative evaluation because use is widespread in this peer group.', evidence:[{label:'Peer prevalence',detail:'31% of Product has active use of this application.',observedAt:'21:30',strength:'observed'},{label:'Unapproved app',detail:'Perplexity AI is not yet approved for this tenant.',observedAt:'21:30',strength:'observed'}] }
];
